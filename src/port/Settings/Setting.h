...
class Base {
public:
    Base(SettingSection section, std::string path, const char* cvar);
    virtual ~Base();

    Base(const Base&) = delete;
    Base& operator=(const Base&) = delete;

    SettingSection Section() const {
        return mSection;
    }
...
public:
    Scalar(SettingSection section, std::string path, V def, Options<V> options = {})
...
// Common instantiations live in Setting.cpp so this header never needs the full JSON parser.
